...
def gibbs_gotovos_sampler(f: Objective, rng: np.random.Generator, cfg: DictConfig) -> Tuple[Counter, List[Set[int]]]:
    """
    ...
    burn_in_ratio = cfg.sampler['gibbs_gotovos'].burn_in_ratio

    # elements dedicated to the burn-in
    n_burn_in = int(M * burn_in_ratio)

    print(f'Running Gibbs-Gotovos sampler with M={M}, burn-in ratio={burn_in_ratio}, n_burn_in={n_burn_in}')

    # run the Gibbs-Gotovos sampler, skipping the initial n_burn_in results
    it: Iterator[Set[int]] = itertools.islice(
        gibbs_gotovos_inner(f=f, rng=rng, M=M + n_burn_in),
        n_burn_in,
        None)

    # chronological history of Gibbs-Gotovos samples
    gibbs_gotovos_history = list(it)

    # aggregate the Gibbs-Gotovos samples
    gibbs_gotovos_samples_f = Counter((frozenset(X) for X in gibbs_gotovos_history))
    return gibbs_gotovos_samples_f, gibbs_gotovos_history


def gibbs_gotovos_inner(f: Objective, rng: np.random.Generator, M: int) -> Iterator[Set[int]]:
    """
    :param f: submodular function
    :param rng: numpy random generator instance
